namespace Moo2U.Droid {
    ...
    public class SqliteService : ISQLite {
        ...
        public SQLiteAsyncConnection GetAsyncConnection() {
            return _sqLiteAsyncConnection ?? (_sqLiteAsyncConnection = new SQLiteAsyncConnection(() => new SQLiteConnectionWithLock(new SQLitePlatformAndroid(), new SQLiteConnectionString(_databasePath, false))));
        }

        public SQLiteConnection GetConnection() {
            return _sqLiteConnection ?? (_sqLiteConnection = new SQLiteConnection(new SQLitePlatformAndroid(), _databasePath, false));
        }
